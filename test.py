...
# date = '2021-01-25' # 2020-10-13 run 0 for successful node agent
# run = '0'
# index = '19999'

# date = '2021-02-01' # 2020-10-13 run 0 for successful node agent
# run = '4'
# index = '19999'
...
def plot_diff_index(date, run):
    plt.figure()
    for index in [str(t) for t in range(4000, 20000, 4000)]:
        ...
        environments = [world.World(graph, randomise_observations=True, shiny=(params['shiny'] if shiny_envs[env_i] else None)) for env_i, graph in enumerate(np.random.choice(envs, n_walks))]
        ...
        filt_size = 11
        plt.plot(analyse.smooth(np.mean(np.array([env for env_i, env in enumerate(correct_model)]),0)[1:], filt_size), label=index)
        # plt.plot(analyse.smooth(np.mean(np.array([env for env_i, env in enumerate(correct_node)]),0)[1:], filt_size), label='node')
        # plt.plot(analyse.smooth(np.mean(np.array([env for env_i, env in enumerate(correct_edge)]),0)[1:], filt_size), label='edge')
        # plt.plot(np.mean(np.array([env for env_i, env in enumerate(correct_model) if envs_to_avg[env_i]]),0)[1:], label='tem')
        # plt.plot(np.mean(np.array([env for env_i, env in enumerate(correct_node) if envs_to_avg[env_i]]),0)[1:], label='node')
        # plt.plot(np.mean(np.array([env for env_i, env in enumerate(correct_edge) if envs_to_avg[env_i]]),0)[1:], label='edge')
    plt.ylim(0, 1)
    plt.legend(fontsize=30, loc=4)
    plt.show()

plot_diff_index(date, run)
...
# Set which environments will include shiny objects
# shiny_envs = [False, False, True, True]
shiny_envs = [False for _ in range(64)]
# Set the number of walks to execute in parallel (batch size)
n_walks = len(shiny_envs)
# Select environments from the environments included in training
environments = [world.World(graph, randomise_observations=True, shiny=(params['shiny'] if shiny_envs[env_i] else None)) for env_i, graph in enumerate(np.random.choice(envs, n_walks))]
...
ax = plot.plot_map(environments[env_to_plot], np.array(occupation[env_to_plot])/sum(occupation[env_to_plot])*environments[env_to_plot].n_locations, min_val=0, max_val=2, ax=None, shape='square', radius=1/np.sqrt(environments[env_to_plot].n_locations))
...
to_plot = np.array(to_acc[0]).reshape([10, 10])
# fig, ax = plt.subplots()
# im = ax.imshow(np.array(to_plot))
...
label = np.array([t['observation'] for t in environments[0].locations]).reshape([10, 10])
for i in range(10):
    for j in range(10):
        text = plt.text(j, i, str(label[i, j]) + "({})".format((label == label[i, j]).sum()), ha="center", va="center", color="w")
