...
accuracy = np.zeros([params["train_epoch"], params["train_it"], 3])

for epoch in trange(params["train_epoch"]):

    logger.info("Epoch: {}, start training in a new environment".format(epoch))
    ...
    for i in trange(i_start, params['train_it']):
        ...
        accuracy[epoch, i, :] = np.array([acc_p, acc_g, acc_gt])
        # Log progress
        if i % 10 == 0:
